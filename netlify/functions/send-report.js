...
const SIGN_OFF = `The tool you just used? I built it from scratch.
...
async function generateIntro(name, companyName, results) {
  ...
  const system = "You write the opening of a personalised email that accompanies a company's SBTi SME (Science Based Targets initiative, small & medium enterprise route) readiness report. Write 3 to 4 sentences of warm, professional plain text. You MUST reference the company by name, state their eligibility outcome, and highlight one specific, concrete takeaway drawn from their actual assessment data (e.g. their emissions totals or chosen target path). Do not invent figures. Do not include a subject line, a sign-off, or a signature — another fixed sign-off is appended after your text. Output only the intro sentences, nothing else.";
